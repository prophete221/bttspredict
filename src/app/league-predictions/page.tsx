      ...
      <main className="flex-1 relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16 w-full">
        <nav aria-label="Fil d'Ariane" className="mb-8">
          <ol className="flex items-center gap-2 text-sm" style={{ color: '#AAB7CC' }}>
            ...
            <li style={{ color: '#F8FAFC' }} aria-current="page">League Predictions</li>
          </ol>
        </nav>
        ...
        <header className="mb-10 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold mt-3 mb-4" style={{ color: '#F8FAFC', fontFamily: 'Poppins, sans-serif' }}>
            League <span style={{ color: '#16C7A3' }}>Predictions</span>
          </h1>
        ...
        <section className="mb-8">
          <h2 className="text-xl font-bold mb-4" style={{ color: '#F8FAFC', fontFamily: 'Poppins, sans-serif' }}>Statistiques par championnat</h2>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr style={{ borderBottom: '1px solid rgba(248, 250, 252, 0.08)' }}>
                  ...
                  <th className="text-left py-3 px-3 font-semibold" style={{ color: '#AAB7CC' }}>Pays</th>
                  <th className="text-right py-3 px-3 font-semibold" style={{ color: '#AAB7CC' }}>Réussite</th>
                  <th className="text-right py-3 px-3 font-semibold" style={{ color: '#AAB7CC' }}>Buts/match</th>
                </tr>
              </thead>
              <tbody>
                {LEAGUES.map((league, i) => (
                  <tr key={i} style={{ borderBottom: '1px solid rgba(248, 250, 252, 0.04)' }}>
                    <td className="py-2 px-3" style={{ color: '#F8FAFC' }}>{league.name}</td>
                    <td className="py-2 px-3" style={{ color: '#AAB7CC' }}>{league.country}</td>
                    <td className="text-right py-2 px-3" style={{ color: '#16C7A3', fontWeight: 700 }}>{league.winRate}</td>
                    <td className="text-right py-2 px-3" style={{ color: '#AAB7CC' }}>{league.avgGoals}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-[10px] mt-2" style={{ color: '#2B3A52' }}>Source : données internes BTTSPredict. Performances passées ≠ garantie future.</p>
        </section>
        ...
        <section className="mb-8 p-6 rounded-2xl" style={{ backgroundColor: 'rgba(22, 199, 163, 0.05)', border: '1px solid rgba(22, 199, 163, 0.15)' }}>
          ...
          <p className="text-sm" style={{ color: '#AAB7CC' }}>
            ...
            Consultez nos <a href="/" style={{ color: '#16C7A3' }}>pronostics du jour</a> ou notre <a href="/historique" style={{ color: '#16C7A3' }}>historique vérifié</a>. Aucun résultat garanti. 18+.
          </p>
        </section>

        <section className="mb-8">
          <h2 className="text-xl font-bold mb-4" style={{ color: '#F8FAFC', fontFamily: 'Poppins, sans-serif' }}>FAQ League Predictions</h2>
