      ...
      <main id="main-content" className="flex-1">
        <article className="max-w-4xl mx-auto px-4 py-12 sm:py-16">
          <nav aria-label="Fil d'Ariane" className="text-xs text-[#B7C4C1] mb-4">
            ...
            <span className="text-[#B7C4C1]">BTTS Statistics</span>
          </nav>
              ...
              <table className="w-full text-sm" style={{ borderCollapse: 'collapse' }}>
                <thead>
                  <tr style={{ borderBottom: '2px solid #5D7880' }}>
                    <th className="text-left py-3 px-4 font-bold text-[#B7C4C1]">Ligue</th>
                    <th className="text-center py-3 px-4 font-bold text-[#B7C4C1]">Taux BTTS</th>
                    <th className="text-center py-3 px-4 font-bold text-[#B7C4C1]">Buts/match</th>
                  </tr>
                </thead>
                <tbody>
                  {LEAGUE_STATS.map((row, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid #5D7880' }}>
                      <td className="py-3 px-4 text-[#F5F8F3] font-medium">{row.league}</td>
                      <td className="text-center py-3 px-4 text-[#B7C4C1] font-mono text-xs">Données en préparation</td>
                      <td className="text-center py-3 px-4 text-[#B7C4C1] font-mono text-xs">Données en préparation</td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section className="mb-10">
            ...
            <div className="space-y-3 text-sm text-[#B7C4C1] leading-relaxed">
              <p>
                <strong className="text-[#F5F8F3]">Taux BTTS</strong> : pourcentage de matchs où les deux équipes ont marqué au moins un but. Les valeurs exactes par ligue seront affichées une fois une source vérifiable intégrée au build.
              </p>
              <p>
                <strong className="text-[#F5F8F3]">Buts par match</strong> : moyenne de buts totaux par match. Plus ce chiffre est élevé, plus le marché Over 2,5 est probable. Les valeurs exactes seront affichées une fois une source vérifiable intégrée au build.
              </p>
              <p>
                Ces statistiques ne garantissent aucun résultat futur. Elles servent uniquement à comprendre la logique de sélection des ligues par le moteur. Pour voir les pronostics actuels, consultez la page <Link href="/btts/predictions/today" className="text-[#B8FF1A] underline">BTTS Predictions Today</Link>.
              </p>
            </div>
          </section>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              <Link href="/btts/predictions/today" className="block p-4 rounded-xl transition-all hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B8FF1A]"
                ...
                <div className="text-sm font-bold text-[#F5F8F3]">BTTS Today →</div>
                <div className="text-xs text-[#B7C4C1] mt-1">Pronostics du jour</div>
              </Link>
              <Link href="/historique" className="block p-4 rounded-xl transition-all hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B8FF1A]"
                style={{ backgroundColor: '#0D1A20', border: '1px solid #5D7880' }}>
                <div className="text-sm font-bold text-[#F5F8F3]">Résultats vérifiés →</div>
                <div className="text-xs text-[#B7C4C1] mt-1">Historique complet</div>
              </Link>
              <Link href="/resultats-verifies" className="block p-4 rounded-xl transition-all hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#B8FF1A]"
                style={{ backgroundColor: '#0D1A20', border: '1px solid #5D7880' }}>
                <div className="text-sm font-bold text-[#F5F8F3]">Vérifications →</div>
                <div className="text-xs text-[#B7C4C1] mt-1">Suivi public</div>
              </Link>
            </div>
          </section>
        </article>
