  ...
  return (
    <div className="min-h-screen bg-[#07131D] flex flex-col text-[#F3F7F5]">
      ...
      <main id="main-content" className="flex-1">
        {/* Breadcrumb */}
        <nav aria-label={copy.breadcrumb} className="max-w-4xl mx-auto px-4 pt-6 pb-2 text-xs text-[#B4C4CC]">
          <Link href={localizedPath('/', locale)} className="hover:text-[#E6A24C]">{copy.home}</Link>
          <span className="mx-1">/</span>
          <Link href={localizedPath('/btts/predictions/today', locale)} className="hover:text-[#E6A24C]">{copy.predictions}</Link>
          <span className="mx-1">/</span>
          <span className="text-[#B4C4CC]">{home} vs {away}</span>
        </nav>

        <article className="max-w-6xl mx-auto px-4 py-8">
          {/* Header */}
          <header className="mb-8 rounded-3xl border border-[#304951] bg-[#0D202D] p-5 shadow-[0_20px_70px_rgba(0,0,0,0.22)] sm:p-8">
            <div className="flex items-center justify-center gap-4 sm:gap-8 mb-4">
              ...
              <div className="text-2xl font-bold text-[#B4C4CC]">vs</div>
              <div className="flex flex-col items-center gap-2 flex-1">
                {awayLogo && (
            ...
            <div className="text-center text-sm text-[#B4C4CC] mb-2">
              {league} · {date}{time ? ` · ${time}` : ''}
            </div>

            {finalScore && (
              <div className="text-center mb-2">
                <span className="inline-block px-4 py-2 rounded-xl text-lg font-bold" style={{ backgroundColor: '#0D202D', border: '1px solid #23495C' }}>
          ...
          <section className="mb-10">
            <div className="mb-6 flex items-end justify-between gap-3">
              <div>
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#E6A24C]">{copy.intelligence}</p>
                <h1 className="mt-1 text-2xl font-bold sm:text-3xl" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  {copy.report}
                </h1>
              </div>
              <span className="text-right text-[10px] uppercase tracking-wider text-[#7F98A4]">{copy.published}</span>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              {marketPredictions.map((p, i) => {
                ...
                const color = isBtts ? '#E6A24C' : isOver ? '#E6A24C' : '#B4C4CC'
                ...
                return (
                  <div key={i} className="p-5 rounded-2xl" style={{ backgroundColor: '#0D202D', border: `1px solid ${color}40` }}>
                    ...
                    <div className="text-2xl font-black mb-2" style={{ color: p.prediction === 'Oui' ? color : '#B4C4CC' }}>
                    ...
                    <div className="text-[10px] text-[#B4C4CC] mb-2">
                      {isBtts ? copy.bttsSubtitle : isOver ? copy.overSubtitle : ''}
                    </div>

                    {isWon && (
                      <div className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold" style={{ backgroundColor: 'rgba(34, 197, 94, 0.15)', color: '#E6A24C' }}>
                        ✓ {copy.won}
                      </div>
                    )}
                    {isLost && (
                      <div className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold" style={{ backgroundColor: 'rgba(255, 113, 133, 0.15)', color: '#E56B6F' }}>
                        ✗ {copy.lost}
                      </div>
                    )}
                    {isPending && (
                      <div className="inline-flex items-center gap-1 px-2 py-1 rounded text-xs font-bold" style={{ backgroundColor: 'rgba(156, 163, 175, 0.15)', color: '#B4C4CC' }}>
                        ⏳ {copy.pending}
                      </div>
                    )}
                  </div>
            ...
            <p className="text-xs text-[#B4C4CC] mt-4 leading-relaxed text-center">
              {copy.noFuture}
            </p>
          </section>
          ...
          {(aiKeyFact || aiExactScore) && (
            <section className="mb-10">
              <div className="rounded-2xl border border-[#304951] bg-[#0D202D] p-5 shadow-[0_18px_50px_rgba(0,0,0,0.18)] sm:p-6">
                <div className="flex items-center justify-between border-b border-[#304951] pb-3">
                  <h2 className="flex items-center gap-2 text-sm font-bold text-[#F3F7F5]">
                    <span className="text-[#E6A24C]">{copy.report}</span> — BTTSPredict AI
                  </h2>
                  {aiExactScore && (
                    <span className="px-2.5 py-1 text-xs font-black rounded-md" style={{ backgroundColor: 'rgba(245,158,11,0.1)', color: '#E6A24C', border: '1px solid rgba(245,158,11,0.3)' }}>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {exactScoreProb && (
                      <div className="text-center rounded-lg p-2" style={{ backgroundColor: 'rgba(245,158,11,0.08)' }}>
                        <div className="text-[9px] uppercase tracking-wider text-[#B4C4CC]">Score exact</div>
                        <div className="text-base font-bold text-[#E6A24C]">{exactScoreProb}</div>
                      </div>
                    )}
                    {aiBttsProb && (
                      <div className="text-center rounded-lg p-2" style={{ backgroundColor: 'rgba(6,182,212,0.08)' }}>
                        <div className="text-[9px] uppercase tracking-wider text-[#B4C4CC]">BTTS</div>
                        <div className="text-base font-bold text-[#E6A24C]">{aiBttsProb}</div>
                      </div>
                    )}
                    {aiOver25Prob && (
                      <div className="text-center rounded-lg p-2" style={{ backgroundColor: 'rgba(255,209,102,0.08)' }}>
                        <div className="text-[9px] uppercase tracking-wider text-[#B4C4CC]">Over 2.5</div>
                        <div className="text-base font-bold text-[#E6A24C]">{aiOver25Prob}</div>
                      </div>
                    )}
                  </div>
                )}
                ...
                {/* Statistique Clé */}
                {aiKeyFact && (
                  <div className="p-3 rounded-lg" style={{ backgroundColor: '#07131D', border: '1px solid #23495C' }}>
                    <span className="text-[11px] font-bold text-[#E6A24C] uppercase tracking-wider block mb-1">
                      📌 {copy.keyFact}
                    </span>
                    <p className="text-xs text-[#F3F7F5] italic">&ldquo;{aiKeyFact}&rdquo;</p>
                  </div>
                )}

                {/* Analyse Complète */}
                {aiAnalysis && (
                  <div className="p-3.5 rounded-lg" style={{ backgroundColor: '#07131D', border: '1px solid #23495C' }}>
                    <span className="text-[11px] font-bold text-[#B4C4CC] uppercase tracking-wider block mb-1">
                      📝 {copy.analysis}
                    </span>
          ...
          <section className="mb-10">
            ...
            <div className="p-4 rounded-xl" style={{ backgroundColor: '#0D202D', border: '1px solid #23495C' }}>
              <div className="grid grid-cols-3 gap-4 text-center">
                <div>
                  <div className="text-2xl font-bold text-[#E6A24C]">{won}</div>
                  <div className="text-xs text-[#B4C4CC] uppercase">{copy.won}</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-[#E56B6F]">{lost}</div>
                  <div className="text-xs text-[#B4C4CC] uppercase">{copy.lost}</div>
                </div>
                <div>
                  <div className="text-2xl font-bold text-[#B4C4CC]">{pending}</div>
                  <div className="text-xs text-[#B4C4CC] uppercase">{copy.pending}</div>
                </div>
              </div>
              {verified.length > 0 && verified[0].verifiedAt && (
                <p className="text-xs text-[#B4C4CC] mt-3 text-center">
              ...
              <p className="text-xs text-[#B4C4CC] mt-2 text-center">
                {copy.source}
              </p>
            </div>
          </section>
          ...
          <section className="mb-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <Link href={localizedPath('/btts/predictions/today', locale)} className="block p-4 rounded-xl transition-all hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E6A24C]"
                style={{ backgroundColor: '#0D202D', border: '1px solid #23495C' }}>
                <div className="text-sm font-bold text-[#F3F7F5] mb-1">{copy.today}</div>
                <div className="text-xs text-[#B4C4CC]">{copy.todayDesc}</div>
              </Link>
              <Link href={localizedPath('/vip', locale)} className="block p-4 rounded-xl transition-all hover:scale-[1.01] focus:outline-none focus-visible:ring-2 focus-visible:ring-[#E6A24C]"
                style={{ backgroundColor: '#0D202D', border: '1px solid #23495C' }}>
                <div className="text-sm font-bold text-[#F3F7F5] mb-1">{copy.premium}</div>
                <div className="text-xs text-[#B4C4CC]">{copy.vipDesc}</div>
              </Link>
            </div>
          </section>
