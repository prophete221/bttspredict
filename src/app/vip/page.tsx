...
const C = {
  bg:'#07111F', surface:'#0D1B2D', border:'#334155',
  text:'#F8FAFC', textSec:'#A7B4C8', textMute:'#A7B4C8',
  baobab:'#38BDF8', data:'#38BDF8', success:'#22C55E', warning:'#D4AF5A',
  gold:'#D4AF5A',
}
...
function formatComboDate(date: string, lang: string) {
  const locale = lang === 'ar' ? 'ar' : lang === 'en' ? 'en-GB' : 'fr-FR'
  return new Intl.DateTimeFormat(locale, { day: '2-digit', month: 'short', year: 'numeric', timeZone: 'Africa/Dakar' })
...
export default function VipPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ backgroundColor: C.bg, color: C.text }}>
      ...
      <main id="main-content" className="vip-dashboard-page flex-1 relative z-10" style={{ paddingBottom: 'calc(80px + env(safe-area-inset-bottom, 0px))' }}>
        ...
        <section data-vip-reveal className="vip-future-hero relative pt-8 pb-6 overflow-hidden">
          ...
          <div className="vip-dashboard-shell relative max-w-[400px] mx-auto px-4">
            {/* Header — VIP Analysis */}
            <div className="vip-dashboard-heading text-center mb-5">
            ...
            <section data-vip-reveal className="vip-combo-deck mb-5 rounded-2xl p-3" style={{ backgroundColor: `${C.surface}CC`, border: `1px solid ${C.gold}35` }} aria-label={vipCopy.dossier}>
              <div className="vip-combo-deck__masthead flex items-center justify-between mb-3">
                <div>
                  ...
                  <h2 className="text-sm font-black uppercase tracking-tight mt-1" style={{ color: C.text }}>{vipCopy.dossier}</h2>
                  <div className="text-[9px] mt-1" style={{ color: C.textSec }}>{vipCopy.dossierIntro} · {vipCopy.comboSource}</div>
                </div>
