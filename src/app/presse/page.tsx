      ...
      <main className="flex-1 relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-12 sm:py-16 w-full">
        <nav aria-label="Fil d'Ariane" className="mb-8">
          <ol className="flex items-center gap-2 text-sm" style={{ color: '#A8B3C2' }}>
            ...
            <li style={{ color: '#F0F2F5' }} aria-current="page">Presse</li>
          </ol>
        </nav>
        ...
        <header className="mb-12 text-center">
          ...
          <h1 className="text-3xl sm:text-4xl font-bold mt-3 mb-4" style={{ color: '#F0F2F5', fontFamily: 'Poppins, sans-serif' }}>
            BTTSPredict dans <span style={{ color: '#00C49A' }}>les médias</span>
          </h1>
        ...
        <div className="space-y-4">
          {MENTIONS.map((mention, i) => (
            <article
              ...
              style={{ backgroundColor: '#161B22', border: '1px solid rgba(240, 242, 245, 0.08)' }}
            >
              <div className="flex items-start justify-between mb-2">
                <div>
                  <span
                    className="text-[10px] px-2 py-0.5 rounded-full font-medium"
                    style={{ backgroundColor: 'rgba(0, 196, 154, 0.08)', color: '#00C49A' }}
                  >
                    {mention.type}
                  </span>
                  <h2 className="text-base font-bold mt-2" style={{ color: '#F0F2F5', fontFamily: 'Poppins, sans-serif' }}>
                    {mention.title}
                  </h2>
                </div>
                <time className="text-xs flex-shrink-0 ml-3" style={{ color: '#6E7681' }}>{mention.date}</time>
              </div>
              <p className="text-xs leading-relaxed mb-3" style={{ color: '#A8B3C2' }}>
                {mention.excerpt}
              </p>
              <div className="flex items-center justify-between">
                <span className="text-xs font-semibold" style={{ color: '#FFD700' }}>{mention.source}</span>
                {mention.url !== '#' && (
                  <a href={mention.url} target="_blank" rel="noopener noreferrer" className="text-xs hover:underline" style={{ color: '#00C49A' }}>
                    Voir la source →
                  </a>
                )}
              </div>
        ...
        {/* Section confiance */}
        <section className="mt-12 p-6 rounded-2xl" style={{ backgroundColor: 'rgba(255, 215, 0, 0.05)', border: '1px solid rgba(255, 215, 0, 0.15)' }}>
          ...
          <div className="text-sm leading-relaxed space-y-2" style={{ color: '#A8B3C2' }}>
            <p>
              <strong style={{ color: '#F0F2F5' }}>Signal d'autorité externe :</strong> Google et les IA (ChatGPT, Google AI Overviews) utilisent les mentions externes pour évaluer la crédibilité d'un site. 32% des citations de Google AI Overviews proviennent de Reddit.
            </p>
            <p>
              <strong style={{ color: '#F0F2F5' }}>Présence organique :</strong> Nos mentions sont naturelles (non promotionnelles) — des utilisateurs partagent leurs analyses et expériences avec BTTSPredict dans les communautés de parieurs.
            </p>
            <p>
              <strong style={{ color: '#F0F2F5' }}>Transparence vérifiable :</strong> Toutes nos performances sont vérifiables publiquement sur notre{' '}
              <a href="/historique" style={{ color: '#00C49A' }}>historique vérifié</a>.
            </p>
          </div>
        </section>
      </main>
