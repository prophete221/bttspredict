    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          style={{ backgroundColor: 'rgba(13, 15, 18, 0.98)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(212, 175, 55, 0.15)' }}
          role="dialog"
          ...
          <div className="max-w-[440px] sm:max-w-2xl mx-auto">
            <div className="flex flex-col sm:flex-row items-start gap-4 mb-4">
              <div className="flex items-start gap-3 flex-1">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(212, 175, 55, 0.1)' }}>
            ...
            <AnimatePresence>
              {customize && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                  <div className="rounded-xl p-4 mb-4 space-y-3" style={{ backgroundColor: 'rgba(30, 34, 40, 0.6)', border: '1px solid rgba(244, 247, 250, 0.08)' }}>
                      ...
                      <label key={cookie.id} className="flex items-start gap-3 cursor-pointer group">
                        <div className="pt-0.5">
                          ...
                          <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all"
                            style={{
                              backgroundColor: preferences[cookie.id as keyof typeof preferences] ? '#D4AF37' : 'transparent',
                              borderColor: preferences[cookie.id as keyof typeof preferences] ? '#D4AF37' : 'rgba(244, 247, 250,0.2)',
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {cookie.required && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: 'rgba(212, 175, 55,0.15)', color: '#D4AF37' }}>Obligatoire</span>
                            )}
                          </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <div className="flex gap-3 sm:ml-auto order-1 sm:order-2 w-full sm:w-auto">
                <button onClick={handleRefuse}
                  ...
                  style={{ border: '1px solid rgba(244, 247, 250,0.1)', color: '#A8B3C7', backgroundColor: 'transparent' }}
                ...
                {customize && (
                  <button onClick={handleSavePreferences}
                    className="flex-1 sm:flex-initial px-5 py-2.5 text-sm rounded-xl font-medium transition-all"
                    style={{ border: '1px solid rgba(212, 175, 55,0.3)', color: '#D4AF37', backgroundColor: 'transparent' }}
                  >
                    Enregistrer
                  </button>
                )}
                <button onClick={handleAccept}
                  ...
                  style={{
                    ...
                    color: '#0D0F12',
                    boxShadow: '0 0 0 1px rgba(212, 175, 55,.3), 0 4px 16px rgba(212, 175, 55,.2)',
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.boxShadow = '0 0 0 1px rgba(212, 175, 55,.5), 0 8px 24px rgba(212, 175, 55,.3)'}
                  onMouseLeave={(e) => e.currentTarget.style.boxShadow = '0 0 0 1px rgba(212, 175, 55,.3), 0 4px 16px rgba(212, 175, 55,.2)'}
                >
                  Accepter
                </button>
              </div>
