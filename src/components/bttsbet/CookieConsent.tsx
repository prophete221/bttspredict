    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          style={{ bottom: 'calc(64px + env(safe-area-inset-bottom, 0px))', backgroundColor: '#0D202D', backdropFilter: 'blur(16px)', borderTop: '1px solid #3A647A', maxHeight: 'calc(100dvh - 5rem)', overflowY: 'auto' }}
          role="dialog"
          aria-modal="true"
            ...
            <div className="flex flex-col sm:flex-row items-start gap-4 mb-4">
              <div className="flex items-start gap-3 flex-1">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0" style={{ backgroundColor: 'rgba(156, 196, 244, 0.14)' }}>
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#E6A24C" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            <AnimatePresence>
              {customize && (
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.3 }} className="overflow-hidden">
                  <div className="rounded-xl p-4 mb-4 space-y-3" style={{ backgroundColor: '#07131D', border: '1px solid #23495C' }}>
                      ...
                      <label key={cookie.id} className="flex items-start gap-3 cursor-pointer group">
                        <div className="pt-0.5">
                          ...
                          <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all"
                            style={{
                              backgroundColor: preferences[cookie.id as keyof typeof preferences] ? '#E6A24C' : 'transparent',
                              borderColor: preferences[cookie.id as keyof typeof preferences] ? '#E6A24C' : 'rgba(244, 247, 251,0.2)',
                              opacity: cookie.required ? 0.7 : 1,
                            }}>
                            {preferences[cookie.id as keyof typeof preferences] && (
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#07131D" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                            )}
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {cookie.required && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: 'rgba(156, 196, 244, 0.16)', color: '#9896FF' }}>{lang === 'fr' ? 'Obligatoire' : lang === 'en' ? 'Required' : 'مطلوب'}</span>
                            )}
                          </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <div className="flex gap-3 sm:ml-auto order-1 sm:order-2 w-full sm:w-auto">
                <button onClick={handleRefuse}
                  ...
                  style={{ border: '1px solid #3A647A', color: '#B4C4CC', backgroundColor: 'transparent' }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#142D3D'; e.currentTarget.style.color = '#F3F7F5' }}
                  onMouseLeave={(e) => { e.currentTarget.style.backgroundColor = 'transparent'; e.currentTarget.style.color = '#B4C4CC' }}
                >
                ...
                {customize && (
                  <button onClick={handleSavePreferences}
                    className="flex-1 sm:flex-initial px-5 py-2.5 text-sm rounded-xl font-medium transition-all"
                    style={{ border: '1px solid #E6A24C', color: '#E6A24C', backgroundColor: 'transparent' }}
                  >
