    ...
    <AnimatePresence>
      {show && (
        <motion.div
          ...
          style={{ backgroundColor: 'rgba(10, 15, 30, 0.98)', backdropFilter: 'blur(16px)', borderTop: '1px solid rgba(22, 163, 74, 0.15)' }}
          role="dialog"
                      ...
                      <label key={cookie.id} className="flex items-start gap-3 cursor-pointer group">
                        <div className="pt-0.5">
                          ...
                          <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all"
                            style={{
                              backgroundColor: preferences[cookie.id as keyof typeof preferences] ? '#16A34A' : 'transparent',
                              borderColor: preferences[cookie.id as keyof typeof preferences] ? '#16A34A' : 'rgba(255,255,255,0.2)',
                              opacity: cookie.required ? 0.7 : 1,
                            }}>
                            {preferences[cookie.id as keyof typeof preferences] && (
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#020617" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                            )}
                          </div>
                        </div>
              ...
              <div className="flex gap-3 sm:ml-auto order-1 sm:order-2 w-full sm:w-auto">
                ...
                {customize && (
                  <button onClick={handleSavePreferences}
                    className="flex-1 sm:flex-initial px-5 py-2.5 text-sm rounded-xl font-medium transition-all"
                    style={{ border: '1px solid rgba(0,229,255,0.3)', color: '#16A34A', backgroundColor: 'transparent' }}
                  >
                    Enregistrer
                  </button>
                )}
