                      ...
                      <label key={cookie.id} className="flex items-start gap-3 cursor-pointer group">
                        <div className="pt-0.5">
                          ...
                          <div className="w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all"
                            style={{
                              ...
                              borderColor: preferences[cookie.id as keyof typeof preferences] ? '#D4AF37' : 'rgba(247, 248, 255,0.2)',
                              opacity: cookie.required ? 0.7 : 1,
                            }}>
                            {preferences[cookie.id as keyof typeof preferences] && (
                              <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#070A14" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                            )}
                          </div>
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {cookie.required && (
                              <span className="text-[10px] px-2 py-0.5 rounded-full font-medium" style={{ backgroundColor: 'rgba(81, 70, 245,0.15)', color: '#D4AF37' }}>Obligatoire</span>
                            )}
                          </div>
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3">
              ...
              <div className="flex gap-3 sm:ml-auto order-1 sm:order-2 w-full sm:w-auto">
                <button onClick={handleRefuse}
                  ...
                  style={{ border: '1px solid rgba(247, 248, 255,0.1)', color: '#94A3B8', backgroundColor: 'transparent' }}
                  onMouseEnter={(e) => { e.currentTarget.style.backgroundColor = '#111827'; e.currentTarget.style.color = '#F1F5F9' }}
                ...
                {customize && (
                  <button onClick={handleSavePreferences}
                    className="flex-1 sm:flex-initial px-5 py-2.5 text-sm rounded-xl font-medium transition-all"
                    style={{ border: '1px solid rgba(81, 70, 245,0.3)', color: '#D4AF37', backgroundColor: 'transparent' }}
                  >
                    Enregistrer
                  </button>
                )}
