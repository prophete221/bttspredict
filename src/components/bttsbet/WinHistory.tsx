...
export default function WinHistory() {
    ...
    return (
      <section id="win-history" className="py-12 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <div className="inline-block w-8 h-8 border-2 border-[#9CC4F4]/30 border-t-[#9CC4F4] rounded-full animate-spin" />
        </div>
      </section>
    )
  }
  ...
  if (!data || !data.stats) {
    return (
      <section id="win-history" className="py-12 px-4">
        <div className="max-w-2xl mx-auto text-center">
          <p className="text-sm text-[#9BA7B8]">Données non disponibles.</p>
        </div>
      </section>
    )
  }
      ...
      <div className="max-w-2xl mx-auto space-y-4">
        ...
        <div className="text-center mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#F7F4EE]" style={{ fontFamily: 'Poppins, sans-serif' }}>
            Historique Vérifié — <span className="text-[#9CC4F4]">{stats.total} matchs vérifiés via ESPN</span>
          </h2>
        </div>
        ...
        <div className="rounded-[16px] bg-[#192231] border border-[#3A4556]/50 p-3 flex items-center justify-between text-xs text-[#9BA7B8]">
          <span className="flex items-center gap-2">
            <span className="w-2 h-2 bg-[#9CC4F4] rounded-full animate-pulse"></span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div className="rounded-[16px] bg-[#111722] border border-[#3A4556] p-5" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.22)' }}>
            <div className="text-[10px] text-[#9BA7B8] uppercase tracking-widest">Tous les pronos</div>
            <div className="mt-1 flex items-baseline gap-2">
              <div className="text-3xl font-bold text-white font-mono">{stats.rate}%</div>
              <div className="text-xs text-[#9BA7B8]">{stats.won}W/{stats.lost}L</div>
            </div>
            ...
            <div className="mt-3 h-1.5 bg-[#080B12] rounded-full">
              <div className="h-full bg-[#9CC4F4] rounded-full transition-all duration-700" style={{ width: `${stats.rate}%` }}></div>
            </div>
          </div>
          ...
          {hasGold && goldReady && (
            <div className="rounded-[16px] bg-[#192231] border border-[#9CC4F4]/42 p-5" style={{ boxShadow: '0 10px 30px rgba(0,0,0,0.22)' }}>
              ...
              <div className="mt-3 h-1.5 bg-[#080B12] rounded-full">
                <div className="h-full bg-[#9CC4F4] rounded-full transition-all duration-700" style={{ width: `${gold.rate}%` }}></div>
              </div>
            </div>
          )}
          {hasGold && !goldReady && (
        ...
        {/* Footer */}
        <div className="text-[11px] text-[#9BA7B8] px-1">
          Source: ESPN public • Vérifiable match par match ci-dessous •{' '}
          <Link href="/methodologie" className="underline hover:text-[#9BA7B8]">Comment on vérifie?</Link>
        </div>

        {/* Trend 14j */}
        {stats.trend14 && stats.trend14.length > 0 && (
          <div className="rounded-[16px] bg-[#111722] border border-[#3A4556] p-4">
            <h3 className="text-sm font-bold text-[#F7F4EE] mb-3">Tendance 14 jours</h3>
                ...
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    ...
                    style={{
                      ...
                      backgroundColor: d.rate >= 55 ? '#9CC4F4' : d.rate >= 40 ? '#9CC4F4' : '#C95A62',
                      minHeight: '4px',
                    }}
                    title={`${d.date}: ${d.rate}% (${d.won}W/${d.lost}L)`}
                  />
                  <span className="text-[10px] text-[#9BA7B8]">{d.date.slice(5)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {data.history && data.history.length > 0 && (
          <div className="rounded-[16px] bg-[#111722] border border-[#3A4556] p-4">
            <h3 className="text-sm font-bold text-[#F7F4EE] mb-3">Derniers résultats vérifiés</h3>
            <div className="space-y-1 max-h-96 overflow-y-auto">
              {data.history.slice(0, 30).map((h: any, i: number) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-2 text-[11px] py-1.5 border-b border-[#3A4556]/30">
                    <span className="text-[#9BA7B8] font-mono flex-shrink-0">{(h.date || '').slice(5)}</span>
                    <span className="text-[#9BA7B8] truncate flex-1">{h.match}</span>
                    <span className="text-[#9BA7B8] flex-shrink-0">{market}</span>
                    ...
                    <span
                      className={`px-1.5 py-0.5 rounded text-[9px] font-bold flex-shrink-0 ${
                        isWon ? 'bg-[#9CC4F4]/14 text-[#9CC4F4]' : 'bg-[#C95A62]/14 text-[#C95A62]'
                      }`}
                    >
                      {isWon ? 'W' : 'L'}
                    </span>
                    {isGold && (
                      <span className="px-1.5 py-0.5 rounded text-[10px] font-bold bg-[#9CC4F4]/14 text-[#9CC4F4] flex-shrink-0">
                        GOLD
                      </span>
                    )}
                  </div>
        ...
        {/* Disclaimer */}
        <div className="text-[10px] text-[#9BA7B8] text-center pt-2">
