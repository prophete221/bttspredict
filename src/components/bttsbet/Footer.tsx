...
export default function Footer({ initialLocale }: { initialLocale?: Locale } = {}) {
  ...
  return (
    <>
      <footer ref={ref} id="faq" className="border-t pt-10 pb-20 sm:pb-8 px-4" style={{ borderColor: '#E6A24C', backgroundColor: '#07131D' }}>
        <div className="max-w-[440px] sm:max-w-2xl mx-auto">
          ...
          <motion.div initial="hidden" className="mb-6">
            <div className="text-center mb-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#E6A24C]">{t.common.transparency}</span>
            </div>
            <div className="p-4 rounded-xl text-center" style={{ backgroundColor: '#0D202D', border: '1px solid rgba(244, 247, 251, 0.08)' }}>
              <p className="text-[11px] text-[#B4C4CC] leading-relaxed">
                {lang === 'fr' ? (
                  <>BTTSPredict ne publie pas de témoignages clients. Notre engagement de transparence repose sur un <a href={localizedPath('/resultats-verifies', lang)} className="text-[#E6A24C] underline">historique vérifiable publiquement</a>, une <a href={localizedPath('/methodologie', lang)} className="text-[#E6A24C] underline">méthodologie documentée</a> et un suivi public lancé le 2026-08-08. Aucun résultat futur n’est garanti.</>
                ) : lang === 'en' ? (
                  <>BTTSPredict does not publish customer testimonials. Our transparency commitment is based on a <a href={localizedPath('/resultats-verifies', lang)} className="text-[#E6A24C] underline">publicly verifiable history</a>, a <a href={localizedPath('/methodologie', lang)} className="text-[#E6A24C] underline">documented method</a> and public tracking. No future result is guaranteed.</>
                ) : (
                  <>لا تنشر BTTSPredict شهادات العملاء. تعتمد الشفافية على <a href={localizedPath('/resultats-verifies', lang)} className="text-[#E6A24C] underline">سجل عام موثق</a> و<a href={localizedPath('/methodologie', lang)} className="text-[#E6A24C] underline">منهجية موثقة</a> وتتبع علني. لا توجد ضمانات لأي نتيجة مستقبلية.</>
                )}
              </p>
            </div>
          </motion.div>
          ...
          <motion.div initial="hidden" className="mb-6">
            ...
            <div className="space-y-2">
              {(lang === 'fr' ? FAQ_ITEMS.slice(0, 4) : t.faqItems).map((item, i) => (
                <div key={item.q} className="rounded-xl overflow-hidden" style={{ backgroundColor: '#0D202D', border: '1px solid rgba(244, 247, 251, 0.08)' }}>
                  ...
                  {openFaq === i && (
                    <div className="px-4 pb-3 text-[11px] text-[#E6A24C] leading-relaxed">{item.a}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
            {legalLinks.map(link => (
              <a key={link.label} href={localizedPath(link.href, lang)} className="text-center text-[10px] text-[#B4C4CC] hover:text-[#E6A24C] transition-colors py-2">
                {link.label}
              </a>
            ))}
          </div>
          ...
          {/* Legal text */}
          <div className="rounded-xl p-4 mb-3" style={{ backgroundColor: '#0D202D' }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[#E6A24C] font-extrabold text-xs">18+</span>
              <span className="text-[10px] text-[#B4C4CC]">| {LONASE.name} | {t.common.responsible}</span>
            </div>
            <p className="text-[10px] text-[#B4C4CC] leading-relaxed">
              <strong className="text-[#E6A24C]">{t.common.warning} :</strong> {lang === 'fr' ? LEGAL.disclaimer : t.legal.risk + ' ' + t.legal.noGuarantee + ' ' + t.legal.eighteen}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            <a href="https://twitter.com/bttspredict" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B4C4CC"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <a href="https://www.facebook.com/bttspredict" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B4C4CC"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073"/></svg>
            </a>
            <a href="https://www.instagram.com/bttspredict" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B4C4CC"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z"/></svg>
            </a>
            ...
            <a href="https://www.youtube.com/@bttspredict" target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B4C4CC"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>
            </a>
          </div>

          {/* Affiliation disclaimer */}
          <p className="text-center text-[10px] text-[#B4C4CC] mb-2 leading-relaxed">
          ...
          <div className="text-center text-[10px] text-[#B4C4CC] mt-3 space-y-1">
            <div>{t.common.publisher} · Contact conformité: {' '}
              <a href="mailto:contact@bttspredict.com" className="underline hover:text-[#B4C4CC]">contact@bttspredict.com</a>
            </div>
            <div>{t.legal.noGuarantee} — 18+ {t.common.responsible}</div>
          </div>

          <div className="text-center text-[10px] text-[#23495C] mt-2">
            {LEGAL.copyright}
          </div>
        </div>
      </footer>
