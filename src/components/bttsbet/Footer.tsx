        ...
        <div className="max-w-[440px] sm:max-w-2xl mx-auto">
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate={isVisible ? 'visible' : 'hidden'} className="mb-6">
            <div className="text-center mb-3">
              <span className="text-[10px] font-bold uppercase tracking-[0.15em] text-[#00FF88]">Ils utilisent BTTSPredict</span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {TESTIMONIALS.slice(0, 3).map((t) => (
                <motion.div key={t.name + t.city} variants={staggerChildFadeUp} className="rounded-xl p-4" style={{ backgroundColor: '#141414', border: '1px solid rgba(255,255,255,0.06)' }}>
                  <div className="flex items-center gap-2.5 mb-2.5">
                    <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: 'rgba(0,255,136,0.1)', color: '#00FF88' }}>
                      {t.name.charAt(0)}
                    </div>
                    <div>
          ...
          <motion.div variants={staggerContainer} initial="hidden" animate={isVisible ? 'visible' : 'hidden'} className="mb-6">
            ...
            <div className="space-y-2">
              {FAQ_ITEMS.slice(0, 4).map((item, i) => (
                <motion.div key={item.q} variants={fadeInUp} className="rounded-xl overflow-hidden" style={{ backgroundColor: '#141414', border: '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
            {[
              ...
              <a key={link.label} href={link.href} className="text-center text-[10px] text-[#5a5a5a] hover:text-[#00FF88] transition-colors py-2">
                {link.label}
              </a>
            ))}
          </div>
          ...
          {/* Legal text */}
          <div className="rounded-xl p-4 mb-3" style={{ backgroundColor: '#0f0f0f' }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[#00FF88] font-extrabold text-xs">18+</span>
