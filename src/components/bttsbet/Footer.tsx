      ...
      <footer ref={ref} id="faq" className="border-t pt-10 pb-20 sm:pb-8 px-4" style={{ borderColor: '#16A36A', backgroundColor: '#07111F' }}>
        <div className="max-w-[440px] sm:max-w-2xl mx-auto">
          ...
          <motion.div initial="hidden" className="mb-6">
            ...
            <div className="p-4 rounded-xl text-center" style={{ backgroundColor: '#0D1B2A', border: '1px solid rgba(242, 247, 245, 0.08)' }}>
          ...
          <motion.div initial="hidden" className="mb-6">
            ...
            <div className="space-y-2">
              {FAQ_ITEMS.slice(0, 4).map((item, i) => (
                <div key={item.q} className="rounded-xl overflow-hidden" style={{ backgroundColor: '#0D1B2A', border: '1px solid rgba(242, 247, 245, 0.08)' }}>
                  ...
                  {openFaq === i && (
                    <div className="px-4 pb-3 text-[11px] text-[#16A36A] leading-relaxed">{item.a}</div>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-4">
            {[
              ...
              <a key={link.label} href={link.href} className="text-center text-[10px] text-[#B7C7D9] hover:text-[#16A36A] transition-colors py-2">
                {link.label}
              </a>
            ))}
          </div>
          ...
          {/* Legal text */}
          <div className="rounded-xl p-4 mb-3" style={{ backgroundColor: '#0D1B2A' }}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[#16A36A] font-extrabold text-xs">18+</span>
              <span className="text-[10px] text-[#B7C7D9]">| {LONASE.name} | Jeu responsable</span>
            </div>
            <p className="text-[10px] text-[#B7C7D9] leading-relaxed">
              <strong className="text-[#16A36A]">Avertissement :</strong> {LEGAL.disclaimer}
            </p>
          </div>
          ...
          <div className="flex items-center justify-center gap-3 mb-4">
            <a href="https://twitter.com/bttspredict" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B7C7D9"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
            </a>
            <a href="https://www.facebook.com/bttspredict" target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="opacity-60 hover:opacity-100 transition-opacity">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#B7C7D9"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073"/></svg>
            </a>
          ...
          <p className="text-center text-[10px] text-[#B7C7D9] mb-2 leading-relaxed">
          ...
          <div className="text-center text-[10px] text-[#B7C7D9] mt-3 space-y-1">
            <div>Éditeur: BTTSPredict · Dakar, Sénégal · Contact conformité: {' '}
              <a href="mailto:contact@bttspredict.com" className="underline hover:text-[#B7C7D9]">contact@bttspredict.com</a>
            </div>
            <div>Les performances passées ne préjugent pas des résultats futurs — 18+ Jouez responsable</div>
          </div>

          <div className="text-center text-[10px] text-[#29445F] mt-2">
            {LEGAL.copyright}
          </div>
        </div>
      </footer>
