      ...
      <div className="max-w-[440px] sm:max-w-2xl mx-auto">
        ...
        >
          {STATS.map((stat, i) => (
            <div key={stat.label} className="stat-tile">
              ...
              <div className="text-xl sm:text-2xl font-bold text-papier tabular-nums">{stat.value}</div>
              <div className="text-[10px] text-cendre uppercase tracking-widest font-medium mt-1">
                {stat.label}
              </div>
            </div>
          ))}
        ...
        >
          <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
            <h3 className="text-lg font-bold text-papier">Présence panafricaine</h3>
            <span className="text-xs text-cendre">{COUNTRIES.length} pays · expansion continue</span>
          </div>
              ...
              >
                <div className="flex items-start justify-between gap-3 mb-2">
                  <div className="flex-1">
                    ...
                    <h3 className="text-base font-bold text-papier mt-1">{item.title}</h3>
                  </div>
                  <span className={`badge ${status.class} text-[9px]`}>{status.label}</span>
                </div>
                <p className="text-xs text-cendre leading-relaxed">{item.description}</p>
              </div>
            )
          })}
        </div>
