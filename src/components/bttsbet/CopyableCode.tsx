    ...
    return (
      <span className={`inline-flex items-center gap-1.5 ${className}`}>
        <motion.button
          ...
          style={{
            background: 'linear-gradient(100deg, #FFD700 0%, #FFE066 30%, #FFD700 50%, #FFE066 70%, #FFD700 100%)',
        ...
        <motion.button
          ...
          style={{ color: copied ? '#00C49A' : '#FFD700' }}
          aria-label="Copier"
        >
          {copied ? (
  ...
  return (
    <span className={`inline-flex items-center gap-1 ${className}`}>
      <motion.button
        ...
        aria-label={`Copier le code promo ${code}`}
        style={{ borderBottom: '1px dashed rgba(0, 196, 154, 0.3)' }}
      >
        {code}
      </motion.button>
      <motion.button
        ...
        whileTap={{ scale: 0.9 }}
        className="inline-flex items-center justify-center w-4 h-4 rounded transition-colors"
        style={{ color: copied ? '#00C49A' : '#00C49A' }}
        aria-label="Copier"
      >
        {copied ? (
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12" /></svg>
