...
export default function LanguageSwitcher({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(o => !o)}
        className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-gray-300 hover:text-white hover:bg-dark-800 transition-all border border-edge"
            ...
            >
              {(Object.keys(LANG_LABELS) as Lang[]).map(code => (
                <button
                  ...
                  className={`w-full flex items-center gap-2 px-3 py-2 text-xs hover:bg-dark-800 transition-colors ${
              ...
              <div className="border-t border-edge px-3 py-1.5">
                <p className="text-[10px] text-gray-400">Plus de langues bientôt</p>
              </div>
            </motion.div>
          </>
        )}
