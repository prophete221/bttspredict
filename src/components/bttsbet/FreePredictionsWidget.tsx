...
function PredictionCard({ pred }: { pred: Prediction }) {
  ...
  const probaColor = isBTTS ? '#2176FF' : '#2176FF' // assombri cyan #2176FF → #2176FF
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        ...
        e.currentTarget.style.boxShadow = '0 0 20px rgba(199, 244, 100, 0.3), 0 8px 32px rgba(7, 17, 26, 0.5)'
      }}
      onMouseLeave={(e) => {
  ...
  if (!src || err) {
    ...
    return (
      <span
        ...
        style={{ backgroundColor: '#0D1B2A', color: '#B7C7D9' }}
        aria-label={alt}
        title={alt}
      >
        {initials}
